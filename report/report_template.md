...
运行 `python -m src.engine.demo --checkpoint checkpoints/gru_best.pt`，将 `runs/gru_base/demo/samples.md` 中的五组首句续写和五组藏头结果粘贴到此处。逐组分析：结构是否完整、是否跑题、是否有重复、藏头是否自然、是否疑似复写训练集。
