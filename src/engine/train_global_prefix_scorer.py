...
from src.data.global_prefix_scorer_dataset import GlobalPrefixMaskDataset, collate_global_prefix_examples
...
def run_epoch(model, loader, device, optimizer=None, grad_clip: float = 1.0):
    ...
    for batch in tqdm(loader, leave=False):
        ...
        with torch.set_grad_enabled(training):
            ...
            weighted_loss = (losses * target_weights).sum() / target_weights.sum().clamp(min=1.0)
            if training:
...
def save_checkpoint(path: Path, model: GlobalPrefixBiGRUScorer, vocab: Vocab, config: dict, epoch: int, val_loss: float, val_acc: float) -> None:
    ...
    torch.save(
        {
            ...
            "special_ids": {
                ...
                "prefix_pad_id": model.prefix_pad_id,
                "col_pad_id": model.col_pad_id,
            },
        },
        path,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a prefix-aware BiGRU scorer for chengyu-grid beam search.")
    parser.add_argument("--config", default="configs/global_prefix_bigru_20e.yaml")
    ...
    train_set = GlobalPrefixMaskDataset(train_poems, vocab)
    valid_set = GlobalPrefixMaskDataset(valid_poems, vocab)
    train_loader = DataLoader(
        ...
        collate_fn=collate_global_prefix_examples,
    )
    ...
    model = GlobalPrefixBiGRUScorer(vocab_size=len(vocab.tokens), **config["model"]).to(device)
